...
def get_color(connexion) : 
    query =sql.SQL( 'SELECT couleur COUNT(*) AS total FROM piece GROUP BY couleur GROUP BY total DESC LIMIT 5')
    return execute_select_query(connexion, query)
    
def get_parties(connexion) :
    query =sql.SQL('SELECT idpartie MIN(   ')
    return execute_select_query(connexion, query)
    
def get_3parties(connexion) :
    query =sql.SQL('SELECT p.idpartie, MAX(b.largeur * b.longueur) AS max_dim, COUNT(*) AS nb_pieces_utilisees FROM partiejeu p JOIN tours t ON p.idpartie = t.idpartie JOIN pieces b ON t.id = b.id GROUP BY p.idpartie ORDER BY p.idpartie ORDER BY max_dim DESC, nb_pieces_utilisees DESC LIMIT 3')
    return execute_select_query(connexion, query)
   
def selection_briques (connexion) :
    query =sql.SQL('SELECT id, largeur, longueur, couleur FROM pieces WHERE largeur <= 2 OR longueur <= 2 ORDER BY RAND() LIMIT 4')
    return execute_select_query(connexion, query)
